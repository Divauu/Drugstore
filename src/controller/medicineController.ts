...
const createMedicine = async (req: Request, res: Response) => {
    try {
        ...
        const newMedicine = await prisma.medicine.create({
            data: {
                name, stock, exp_date, type, price, photo
            }
        })
...
const readMedicine = async (re: Request, res: Response) => {
    try {
        const search = re.query.search
        /** get all medicine */
        const allMedicine = await prisma.medicine.findMany({
            where: {OR: [{name: { contains: search?.toString() || ""}},]}})
        return res.status(200).json({
...
const updateMedicine = async (req: Request, res: Response) => {
    try {
        ...
        const { name, stock, exp_date, price, type } = req.body
        ...
        return res.status(200).json({
            message: `Medicine had been updated`,
            data: saveMedicine
        })
    } catch (error) {
        return res.status(500).json(error);
...
export { createMedicine, updateMedicine, readMedicine, deleteMedicine };
...
const createTransaction = async ( req: Request, res: Response ) => {
    try {
        ...
        return res.status(200).json({ message: ` New Transaction has created` })
...
const readTransaction = async ( req: Request, res: Response ) => {
    try {
        ...
        /** menentukan total disetiap transaksi */
        ...
        return res.status(200).json({ message: ` Transaction has been retrieved`, data: allTransactions})
    } catch (error) {
...
const updateTransaction = async ( req: Request, res: Response ) => {
    try {
        ...
        /** check that trasaction exists based on "id" */
        ...
        /** checking medicine (memastikan id obat tersedia) */
        /** kumpulin id obat yang dikirimkan */
        const arrMedicineId = transaction_detail.map( item => item.medicine_id )
        ...
        /** save transaction data */
        const saveTransaction = await prisma.transaction.update({ where: { id: Number(id) }, data: { cashier_name, order_date }
        })

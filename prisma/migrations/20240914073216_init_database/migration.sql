...
CREATE TABLE `transaction_detail` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `transcation_id` INTEGER NOT NULL DEFAULT 0,
    ...
    INDEX `transaction_detail_medicine_id_fkey`(`medicine_id`),
    INDEX `transaction_detail_transcation_id_fkey`(`transcation_id`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `transaction_detail` ADD CONSTRAINT `transaction_detail_transcation_id_fkey` FOREIGN KEY (`transcation_id`) REFERENCES `transaction`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
